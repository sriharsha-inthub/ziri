...
import { ZiriClient } from './ziriClient';
...
function showChatResponse(question: string, response: string) {
  const panel = vscode.window.createWebviewPanel(
    'ziriChat',
    'Ziri Chat',
    vscode.ViewColumn.One,
...
function generateSearchResultsHtml(results: any[], query: string): string {
...
function generateChatHtml(question: string, response: string): string {
  return `
    <!DOCTYPE html>
    <html>
    <head>
